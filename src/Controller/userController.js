...
const registerUser = async (req, res) => {
    try {
        ...
        if (!role || !['author', 'admin', 'user'].includes(role)) { return res.status(400).send({ status: false, error: "missing/invalid parameters role" }); }
        ...
        const isRegisterEmail = await userModel.findOne({ email: email.toLowerCase() });
        console.log("isRegisterEmail",isRegisterEmail)
